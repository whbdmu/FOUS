...
def extract_dy_features(cfg, model, data_loader, device, is_source, memory_proposal_boxes=None,
                        memory_target_features=None, momentum=0.2):
    model.eval()
                ...
                for j in range(len(embeddings)):
                    if memory_proposal_boxes is None:
                        ...
                        if max(ious[-1]) > 0.7:
                            ...
                            memory_target_features[img_store_idx + ious[-1]] = momentum * memory_target_features[
                                img_store_idx + ious[-1]] + (1. - momentum) * embeddings[j]
                            try:
                                memory_proposal_boxes[img_name][ious[-1]] = momentum * memory_proposal_boxes[img_name][
                                    ious[-1]] + (1. - momentum) * boxes[j]
