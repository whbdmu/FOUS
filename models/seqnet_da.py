...
class SeqNetDa(nn.Module):
    ...
    def forward(self, images_s, targets_s=None, images_t=None, targets_t=None, query_img_as_gallery=False, is_source=False, epoch=0):
        ...
        if self.da_heads:
            ...
            if (epoch>=self.target_start_epoch):
                da_ins_labels_t_s = da_ins_labels_t[:,0]
                # da_ins_labels_t_r = da_ins_labels_t[:,1]
                # da_ins_labels_t_index = da_ins_labels_t[:,2]
                da_ins_labels_t_before_s = da_ins_labels_t_before[:,0]
                # da_ins_labels_t_before_r = da_ins_labels_t_before[:,1]
                # da_ins_labels_t_before_index = da_ins_labels_t_before[:,2]
                targets_t_r = deepcopy(targets_t)
                for x, y in zip(targets_t, targets_t_r):
                    x["labels"] = x["labels"][:,0]
                    y["labels"] = y["labels"][:,1]
                da_losses_t = self.da_heads([features_t["feat_res4"]], da_ins_feas_t, da_ins_labels_t_s, da_ins_feas_t_before, da_ins_labels_t_before_s, targets_t)
                #da_losses_t_r = self.da_heads([features_t["feat_res4"]], da_ins_feas_t, da_ins_labels_t_r, da_ins_feas_t_before, da_ins_labels_t_before_r, targets_t_r)
                da_losses_t["loss_da_image_t"] = da_losses_t.pop("loss_da_image") # + da_losses_t_r.pop("loss_da_image")
                da_losses_t["loss_da_instance_t"] = da_losses_t.pop("loss_da_instance") # + da_losses_t_r.pop("loss_da_instance")
                da_losses_t["loss_da_consistency_t"] = da_losses_t.pop("loss_da_consistency") # + da_losses_t_r.pop("loss_da_consistency")
